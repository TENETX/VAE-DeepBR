...
def all_metrics(outputs, pre_labels, true_labels):
    ...
    metrics_res = [hamming_loss, avg_precision, one_error, ranking_loss, coverage, macro_f1, micro_f1,
                   macro_precision, micro_precision, macro_recall, micro_recall]
    # metrics_res = [hamming_loss, avg_precision, one_error, ranking_loss, coverage]
...
def MacroF1(pre_labels, true_labels):
    ...
    maf = 0
    for i in range(q):
        tp = np.sum(((pre_labels[:, i]) & (true_labels[:, i])) is True)
        fp = np.sum((pre_labels[:, i] & (1 - true_labels[:, i])) is True)
        fn = np.sum(((1 - pre_labels[:, i]) & true_labels[:, i]) is True)
        if tp + fp + fn == 0:
            tmp_maf = 0
        else:
...
def MicroF1(pre_labels, true_labels):
    tp = np.sum(((pre_labels) & (true_labels)) is True)
    fp = np.sum((pre_labels & (1 - true_labels)) is True)
    fn = np.sum(((1 - pre_labels) & true_labels) is True)
    if (tp + fp + fn) == 0:
        mif = 0
    else:
        mif = (2 * tp) / (2 * tp + fp + fn)
...
def cm_precision_recall(prediction, truth):
    ...
    cm = np.array([confusion_matrix[True, True], confusion_matrix[False, False],
